...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param)
{
	...
	else
	{
		...
		int max_nr_class = 16;
		int nr_class = 0;
		int *label = Malloc(int,max_nr_class);//样本的标志类
		int *count = Malloc(int,max_nr_class);//每一类样本的个数
		int *index = Malloc(int,l);//对样本重新标志
		...
		svm_node **x = Malloc(svm_node *,l);//复制训练样本集
...
int svm_save_model(const char *model_file_name, const svm_model *model)
{
	...
	fprintf(fp, "SV\n");
	const double * const *sv_coef = (const double *const *) model->sv_coef;
	const svm_node * const *SV = (const svm_node *const *) model->SV;
