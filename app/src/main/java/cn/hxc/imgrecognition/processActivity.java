...
public class processActivity extends Activity {
	...
	public native int callint(int[] by1, int w, int h, String num, String win2, String whi2, int flag);
...
//			height=tempHeight;
//		}
//		processUtil.removeMaxNoise(a,width,height,id);
       // 合并同一个字符
...
//		//TODO //对过大连通元重新进行处理
	...
	public void recoginze() throws IOException {
		...
		//for (int i = 0; i <=count-1; i++) {
			if (sorta[i * 7] == 0) {
				...
				if (wAh < 0.3 || ((percent == processUtil.averagePercent) && (wAh < 0.45))
					...
					num =callint(by1, numWidth, numHeight, strNum, strWin2, strWhi2, 0);
					if (percent > 0.55) {
						if (num == 5) {
							if (pix3 < numHeight / 2) {
								num = 3;
		...
		// if (subHeightCount > 4 || alignHeight.size() < 7) {
